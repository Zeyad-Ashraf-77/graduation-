...
export default function AdminDashboard() {
  ...
  async function getProducts() {
    try {
      setIsLoading(true);
      const { data } = await axios.get(
        `https://project1-kohl-iota.vercel.app/product`,
        {
          headers: { Authorization: localStorage.getItem("authorization") },
        }
      );
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8 bg-[#f9f9f6] dark:bg-gray-900 text-gray-800 dark:text-gray-100 min-h-screen">
      <h1 className="mt-20 text-[#4e342e] text-center dark:text-amber-400 font-bold">
        Admin Dashboard
      </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div
          ...
          className="rounded-2xl hover:scale-108 transition-all duration-150 shadow-lg shadow-yellow-500 bg-stone-100 dark:bg-gray-800 dark:border dark:border-amber-400 p-4 text-center"
        >
          <p className="text-4xl  text-[#4e342e] dark:text-amber-500 font-bold">
            {productsDs.length}
          </p>
        ...
        <div
          ...
          className="rounded-2xl hover:scale-105 transition-all shadow-lg shadow-yellow-500 bg-stone-100 dark:bg-gray-800 p-4 text-center"
        >
          <p className="text-4xl  text-[#4e342e] dark:text-amber-500 font-bold">
            {categoryNum}
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-stone-100  shadow-xl shadow-yellow-500 dark:bg-gray-900 dark:border dark:border-amber-400 rounded-2xl overflow-x-auto">
          ...
          <div className="max-h-[400px] overflow-y-auto">
            <table
              onClick={() => navigate("/orders")}
              className="min-w-full cursor-pointer text-sm"
            >
              <thead className="border-b sticky top-0 bg-stone-100 dark:bg-gray-900">
                <tr>
                  <th className="text-left text-[#4e342e] dark:text-amber-500 py-2 px-2">
                    Order ID
                  </th>
                  <th className="text-left text-[#4e342e] dark:text-amber-500 py-2 px-2">
                    Date
